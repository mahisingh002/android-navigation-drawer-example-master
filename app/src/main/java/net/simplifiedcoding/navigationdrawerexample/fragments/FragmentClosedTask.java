...
public class FragmentClosedTask extends Fragment implements Callback<NewTaskData>, AdapterNewTask.ClickListener {

    private static final String TAG = FragmentClosedTask.class.getSimpleName();
    ...
    public FragmentClosedTask() {
        // Required empty public constructor
    }


    public static FragmentClosedTask newInstance() {
        FragmentClosedTask fragment = new FragmentClosedTask();
    ...
    void initView(View view) {
        ...
        tvTitleName.setText(getString(R.string.closed_task));
        ...
        if (newTaskData != null) {
            ...
            if (Integer.parseInt(newTaskData.getStatus()) == Constant.StatusCode.CLEANMONEY_CODE_SUCCESS) {
                if (newTaskData.getData() != null && newTaskData.getData().size() > 0) {
                    adapterNewTask = new AdapterNewTask(getContext(), newTaskData.getData(), "close", color_value,Todo_monitering);
                    recyclerView.setAdapter(adapterNewTask);
                    adapterNewTask.setOnItemClickListener(FragmentClosedTask.this);
                } else {
                    no_task_tv.setText(getString(R.string.no_closedtask));
